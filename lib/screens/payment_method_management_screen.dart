...
class _PaymentMethodManagementScreenState extends State<PaymentMethodManagementScreen> {
  ...
  void _savePaymentMethod(PaymentMethod? existingPaymentMethod, String holderName, 
      String cardNumber, String brand, String expiryMonth, String expiryYear) {
    ...
    final newPaymentMethod = PaymentMethod(
      ...
      expiryYear: expiryYear.padLeft(2, '0'),
    ...
    final user = authController.currentUser!;
    List<PaymentMethod> updatedPaymentMethods = List.from(user.paymentMethods);
  ...
  void _setAsDefault(PaymentMethod paymentMethod) {
    final user = authController.currentUser!;
    List<PaymentMethod> updatedPaymentMethods = user.paymentMethods
        .map((pm) => pm.copyWith(isDefault: pm.id == paymentMethod.id))
        .toList();
...
class _PaymentMethodFormScreenState extends State<_PaymentMethodFormScreen> {
  ...
  void initState() {
    ...
    _expiryMonthController = TextEditingController(text: widget.paymentMethod?.expiryMonth ?? '');
    _expiryYearController = TextEditingController(text: widget.paymentMethod?.expiryYear ?? '');
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                decoration: InputDecoration(
                  ...
                  prefixIcon: Icon(Icons.credit_card, color: Colors.pink[600]),
                  filled: true,
